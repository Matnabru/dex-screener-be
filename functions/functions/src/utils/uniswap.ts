...
import { abi as uniswapPairABI } from "../../src/utils/IUniswapV2Pair.json";
import {
  Transaction,
  Timeframe,
  TransactionType,
...
const web3 = new Web3(
  new Web3.providers.HttpProvider(
    "https://mainnet.infura.io/v3/fe053da3410d481f8537499a08bdfe7b",
  ),
);

function convertToFloat(bigNumber: BigNumber, decimals: number) {
...
export async function swapEventsWithPriceUsingBlock(
  contractAddress: string,
  lastBlock: number,
): Promise<TransactionWithEndBlock> {
  ...
  try {
    ...
    const pricedEvents = events.map(async (event: any) => {
      ...
      let price;
      let type: TransactionType = TransactionType.Sell;
      let unit;
      let weth;
      ...
      const data: Transaction = {
        type: type,
        unit: unit ? unit.toString() : "0",
        weth: weth ? weth.toString() : "0",
        price: price ? price.toFixed(18) : "0",
    ...
    const transactions = await Promise.all(pricedEvents.filter((e: any) => e));
...
function groupEventsByTimeframe(
  events: Transaction[],
  seconds: number,
  timeframe: Timeframe,
): Candle[] {
  ...
  let startTime = getStartTimeOfTimeframe(
    new Date(sortedEvents[0].timestamp),
    seconds,
  );
  let currentCandle = createInitialCandle(
    startTime,
    parseFloat(sortedEvents[0].price),
    timeframe,
  );
  ...
  sortedEvents.forEach((event) => {
    ...
    if (eventTimestamp < startTime + seconds * 1000) {
      ...
      while (eventTimestamp >= startTime + seconds * 1000) {
        ...
        currentCandle = createInitialCandle(
          startTime,
          currentCandle.close,
          timeframe,
        );
...
function getStartTimeOfTimeframe(date: Date, seconds: number) {
...
export async function getBlockNumberAfterTimestamp(
  ...
  while (tries < 1000) {
    ...
    if (new BigNumber(block.timestamp.toString()).isGreaterThan(targetTime)) {
      const previousBlock = await web3.eth.getBlock(
        estimatedBlockNumber.minus(1).toNumber(),
      );
      if (
        new BigNumber(previousBlock.timestamp.toString()).isLessThanOrEqualTo(
          targetTime,
        )
      ) {
...
export async function getPairDetails(pairAddress: string): Promise<any> {
  ...
  try {
    ...
    const reserves: any = await pairContract.methods.getReserves().call();
    ...
    if (
      typeof token0Decimals !== "bigint" ||
      typeof token1Decimals !== "bigint"
    ) {
      throw new Error("Token decimals is not a bigint");
    }
    const reserve1adjusted =
      Number(
        (reserves[0] * 1000000000000000n) /
          convertToBiggerBigInt(1n, token0Decimals),
      ) / 1000000000000000;
    const reserve2adjusted =
      Number(
        (reserves[1] * 1000000000000000n) /
          convertToBiggerBigInt(1n, token1Decimals),
      ) / 1000000000000000;
...
import erc20ABI from "../../src/utils/erc20.abi.json";
...
function convertToBiggerBigInt(n: any, powerOf10: any) {
  if (powerOf10 < 0) {
