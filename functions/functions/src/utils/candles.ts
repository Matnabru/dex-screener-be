...
export function combineCandleData(
  ...
  if (
    ...
    newCandles.length > 0 &&
    existingCandles[existingCandles.length - 1].time === newCandles[0].time
  ) {
    ...
    lastExistingCandle.high = Math.max(
      lastExistingCandle.high,
      firstNewCandle.high,
    );
    lastExistingCandle.low = Math.min(
      lastExistingCandle.low,
      firstNewCandle.low,
    );
...
export function groupCandlesByMonth(
  candles: Candle[],
...
export function groupCandlesByYear(
  candles: Candle[],
): Record<string, Candle[]> {
...
export async function storeAndUpdateCandles(db: any, path: any, candles: any) {
  const candleDoc = await db.doc(path).get();
  const existingCandles = candleDoc.exists ? candleDoc.data().candles : [];
